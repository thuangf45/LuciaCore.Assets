export function render(node) {
    ...
    return {
        html: `
            <div id="${nodeId}" class="lucia-wheel-wrapper" style="${node.style || ''}">
                <div class="lucia-wheel-container" data-action="${node.action || ''}">
                    <svg
                        xml:space="preserve"
        ...
        `,
        css: `
            /* Khối bao ngoài chịu lực hệ thống */
            #${nodeId}.lucia-wheel-wrapper {
                display: flex;
                align-items: center;
            ...
            /* Container cô lập không gian chạy của bánh xe và con đường */
            #${nodeId} .lucia-wheel-container {
                width: 120px;
                height: 120px;
                position: relative;
                overflow: hidden;
            ...
            #${nodeId} .wheel {
                ...
                height: 100px;
                border-radius: 50%;
                animation: rolling_${nodeId} 1s linear infinite;
                z-index: 2;
            }

            /* Loại bỏ ID tĩnh, thay bằng scoped class nội bộ */
            #${nodeId} .wheel-rim {
            ...
            /* Con đường chạy phía dưới bánh xe */
            #${nodeId} .road {
                position: absolute;
                bottom: 15px; /* Ghim tọa độ cứng từ đáy */
                left: 0;
                width: 100%;
                height: 8px;
                border-radius: 4px;
                background-color: rgb(55, 55, 55);
                animation: road-move_${nodeId} 2s linear infinite;
                z-index: 1;
            }

            /* ─── HỆ THỐNG KEYFRAMES CÔ LẬP NGUYÊN TỬ ─── */
            @keyframes rolling_${nodeId} {
                to {
                    transform: rotate(-360deg);
                }
            }

            @keyframes road-move_${nodeId} {
                from {
